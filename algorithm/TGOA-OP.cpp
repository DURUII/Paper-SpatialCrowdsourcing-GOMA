...
void TGOA(ifstream& fin, int seqN) {
	...
	for (int id = 0; id < Sequence.size(); ++ id) {
		...
		if (id == Sequence.size() - 1 || nd.begTime != Sequence[id + 1].begTime) {
			// remove workers and tasks after due time.
			for (int i=T_delta.size()-1; i>=0; --i) {
				int tid = T_delta[i];
				if (tasks[tid].endTime < node.begTime) {
					T_delta[i] = *T_delta.rbegin();
					T_delta.pop_back();
				}
			}
			for (int i=W_delta.size()-1; i>=0; --i) {
				int wid = W_delta[i];
				if (workers[wid].endTime < node.begTime) {
					W_delta[i] = *W_delta.rbegin();
					W_delta.pop_back();
				}
			}
...
#ifdef WATCH_MEM
	printf("TGOA-OP %.6lf %.6lf %d\n", utility, usedTime, usedMemory/1024);
#else
	printf("TGOA-OP %.6lf %.6lf\n", utility, usedTime);
#endif
	fflush(stdout);
